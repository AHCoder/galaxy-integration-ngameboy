...
class NintendoGameboyPlugin(Plugin):
    ...
    async def authenticate(self, stored_credentials=None):
        if not stored_credentials:
            PARAMS = {
                "window_title": "Configure Gameboy Integration",
                "window_width": 550,
                "window_height": 730,
    ...
    def _do_auth(self) -> Authentication:
        ...
        return Authentication("mgba_user", user_data["username"])
    ...
    def _launch_game(self, game_id, emu_path, fullscreen) -> None:
        ...
        Interprets user configurated options and launches mGBA with the chosen rom
        '''
    ...
    def _check_emu_status(self) -> None:
        try:
            if self.proc.poll() is not None:
                ...
                self.gb_client._set_session_end()
                session_duration = self.gb_client._get_session_duration()
    ...
    async def _update_local_games(self) -> None:
        ...
        notify_list = self.gb_client._get_state_changes(self.local_games_cache, new_list)
